...
- **????**: `complete` (all required evidence obtained), `partial` (some evidence missing, usable), `failed` (no usable evidence)
- **???**: bullet list in plain Chinese ? describe WHAT was obtained, not HOW (say "????" not "web.extract via Firecrawl")
- **??**: bullet list with impact explanation ? always state what's affected
- **??**: always provide a recommended next step, don't just list options
- **???**: always points to `/promote` for human review
...
| Wrong | Right |
|-------|-------|
| "AgentKey ???????? MediaCrawler Provider?" | "????????????????????1. ??????????? 2. ?????????? 1?" |
| "RapidOCR capability unavailable." | "????????????????????????????????? OCR ?????????????????????" |
| "??? oks capability catalog?" | "???????????????????" (Agent runs it internally) |
...
- **Level 1 (user asks "why this way?")**: "???????????????????????????????"
...
**Never invent numbers.** If `user_impact` says "??" or a descriptive range,
