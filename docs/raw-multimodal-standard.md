...
Level-1 能力入口位于 `scripts/raw_bundle_adapter.py`。Agent 必须先读取 `settings/handlers.json`、明确选择模态和子命令，再调用该 Adapter。插件不承担自动模态探测、跨工具调度、摘要、纠错、评级或知识提升。

本地解释器路径写入未跟踪的 `settings/raw-tools.json`，格式参考 `settings/raw-tools.example.json`。示例：

```powershell
# 仅用于开发诊断；正式 /ingest 路由由 Agent 完成
python scripts/raw_bundle_adapter.py route "D:\sample\lesson.mp4"

# 使用Watch Skill环境执行视频提取并直接生成Raw包
python scripts/raw_bundle_adapter.py watch "D:\sample\lesson.mp4" `
  --source-file "D:\sample\lesson.mp4" `
  --output "raw\2026\07\16\videos\lesson" `
  --max-frames 12

# 运行MarkItDown并生成Office Raw包；也可用--markdown打包已有结果
python scripts/raw_bundle_adapter.py markitdown "D:\sample\slides.pptx" `
  --output "raw\2026\07\16\papers\slides"

# 打包已有MinerU结果
python scripts/raw_bundle_adapter.py mineru "D:\mineru-output" `
  --source "D:\sample\paper.pdf" `
  --output "raw\2026\07\16\papers\paper"

# 使用RapidOCR环境提取独立图片
python scripts/raw_bundle_adapter.py image "D:\sample\screenshot.png" `
  --output "raw\2026\07\16\misc\screenshot"

# 所有包进入raw/前必须通过结构和证据校验
python scripts/raw_bundle_adapter.py validate "raw\2026\07\16\videos\lesson"
```
