...
前面几页讲"应该怎么做"，这一页讲"真做了会怎样"。你会看到知识库如何在每一步替你做判断，以及循环跑完后又如何反过来长出新知识。
...
这条循环被固化在配方 `recipes/oss-contribution-scan.md` 里，边界由
`goals/oss-contribution.md` 划定（竞赛周期、"no-op = 0"、小 PR 优先）。
...
召回一次性端出了积累下来的策略：**选仓库标准**、**静默 merge 模式**（≤40 行 / 1 文件 / 1 issue，实测 75% 直接 merge）、**CI 假阳性排查**，还有项目画像 `rocketmq.md` 里两条血泪教训——
...
## 第 2 步：Gate 1 去重前置——当场拦下 5 个 no-op
...
知识驱动了循环，循环产出了一个 PR **和**一批新知识，新知识又回到知识库——下一轮会跑得更准。这就是把 GitHub 贡献"托管"给知识库的样子。

## 第一步该做什么

1. 给你的贡献目标设一个 goal（周期 + "no-op = 0" 这类硬约束）。
2. 挑一个你熟的仓库，只跑到"产出候选清单"为止——先不提 PR。
3. 感受一下 Gate 1 去重帮你省掉了多少白工。

## 接下来读哪里

* **[自动驾驶](autonomous.md)**：这一轮属于 L2——AI 触发、人类逐条确认。
* **[理念](philosophy.md)**：为什么"教训回流成流程"就是在训练你的知识库。
* **[知识库指标](metrics.md)**：用四个维度看这套循环有没有让知识库变好。
